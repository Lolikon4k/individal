...
@section('content')
    <div class="h-full p-8">
        ...
        <div class="my-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
            <div class="w-full flex mx-auto flex-col 2xl:w-1/3">
                <div class="flex-1 bg-[#2E213F] rounded-lg shadow-xl p-8">
                    <h4 class="text-xl text-white font-bold">Информация об аккаунте</h4>
                    <ul class="mt-2 text-white">
                        <li class="flex border-y py-2">
                            <span class="font-bold w-[150px]">Имя:</span>
                            <span class="text-white">{{Auth::user()->name}} {{Auth::user()->surname}}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-[150px]">День рождение:</span>
                            <span class="text-white">{{Auth::user()->birthday}}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-[150px]">Присоединился:</span>
                            <span class="text-white">{{ \Carbon\Carbon::parse(Auth::user()->created_at)->locale('ru')->isoFormat('D MMM YYYY') }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-[150px]">Телефон:</span>
                            <span class="text-white">{{Auth::user()->phone}}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-[150px]">Почта:</span>
